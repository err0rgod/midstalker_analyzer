...
def get_all_pcap(PCAPS, PD):
    '''
    Process pcap packets and return an ordered mapping for display
    '''
...
def get_filter_pcap(PCAPS, PD, key, value):
    '''
    Filter packets by a specific key/value (e.g., protocol/source/destination)
    '''
    pcaps = collections.OrderedDict()
    count = 1
    for p in PCAPS:
        ...
        if key == 'Procotol':
            ...
            pass
    # Return filtered pcaps in order
    return pcaps


def proto_filter(filter_type, value, PCAPS, PD):
    '''
    Protocol filtering based on selection
    Can filter by "source", "destination" or "protocol"
    '''
...
def showdata_from_id(PCAPS, dataid):
    pcap = PCAPS[dataid]
    # Redirect output to capture pcap.show()
