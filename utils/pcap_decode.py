...
PCAP Decoder Module
==================
Core packet parsing engine for PCAP analysis.

Original Author: dj (Chinese developer)
Enhanced by: err0rgod (Nirbhay Katiyar)
- Translated Chinese comments to English
- Improved code documentation
- Enhanced error handling
- Added comprehensive protocol support

This module provides comprehensive packet decoding capabilities for:
- Ethernet layer protocols
- IP/IPv6 layer protocols  
...
__author__ = 'err0rgod'
__version__ = '2.0'
__description__ = 'Enhanced PCAP Decoder with English documentation'
...
class PcapDecode:
    ...
    Provides comprehensive packet parsing and protocol identification
    for network traffic analysis and cybersecurity investigations.
    """
    
    def __init__(self):
        # Load Ethernet layer protocol configuration file
        ...
        # Load IP layer protocol configuration file
        ...
        # Load application layer protocol port configuration file
        ...
        # Load TCP layer protocol configuration file
        ...
        # Load UDP layer protocol configuration file
    ...
    def ether_decode(self, p):
        """
        Parse Ethernet layer protocol
        
        Args:
            p: Scapy packet object
            
        Returns:
            dict: Parsed packet data with time, source, destination, protocol, length, and info
        """
        data = dict()
        if p.haslayer(Ether):
    ...
    def ip_decode(self, p):
        """
        Parse IP layer protocol (IPv4/IPv6)
        
        Args:
            p: Scapy packet object
            
        Returns:
            dict: Parsed packet data with protocol-specific information
        """
        data = dict()
    ...
    def tcp_decode(self, p, ip):
        """
        Parse TCP layer protocol
        
        Args:
            p: Scapy packet object
            ip: IP layer object (IPv4 or IPv6)
            
        Returns:
            dict: Parsed TCP packet data with port information
        """
        data = dict()
        tcp = p.getlayer(TCP)
        data['time'] = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(int(p.time)))
    ...
    def udp_decode(self, p, ip):
        ...
        Returns:
            dict: Parsed UDP packet data with port information
        """
        data = dict()
        udp = p.getlayer(UDP)
        data['time'] = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(int(p.time)))
